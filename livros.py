...
class LivroSpider(scrapy.Spider):
    ...
    if categoria=='1':
        ...
        def parse(self,response):
            categorias = response.xpath("//ul[contains(@class, 'neemu-category-filter-level neemu-category-filter-level-1 display-block')]/li/a/span[contains(@class,'neemu-filter-text')]//text()")
            cat = []
            for i in categorias:
                opcoes = i.extract()
                cat.append(opcoes)
                print(opcoes)
            #print(cat)
            decisao = int(input('Digite o número da categoria escolhida:'))
